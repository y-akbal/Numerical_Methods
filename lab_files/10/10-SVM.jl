...
mutable struct SVMClassifier <: SVM
    w::AbstractVector
    b::AbstractFloat
    SVMClassifier(n::Int64) = new(randn(n), randn())
end
...
function (svm::SVMClassifier)(x::AbstractMatrix) ##This is for vectors. Let's do it for matrices
...
function fit!(svm::SVMClassifier, 
    ...
    for i in 1:max_iter
        val, grad_svm = Zygote.withgradient(svm) do svm
            0.5*norm(svm.w)^2+C*mean(max.(1 .- y.*(X*svm.w .+ svm.b), ξ))
        end

        _, grad_ξ = Zygote.withgradient(ξ) do ξ
            0.5*norm(svm.w)^2+C*mean(max.(1 .- y.*(X*svm.w .+ svm.b), ξ))
        end
        ...
        ## gradients with respect to ξ can not be obtained by zygote directly, so we do 
        ## it seperately!!!
        ## We update w, b, and ξ
...
begin
    ...
    fit!(svm, X_train, y_train; max_iter = 10000, lr = 1e-4)
end
