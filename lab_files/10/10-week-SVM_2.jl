...
function sep(q::AbstractFloat)
    return ifelse(q > 0, 1, -1)
end

function (svm::SVMClassifier)(x::AbstractVector)
    return transpose(x)*svm.w + svm.b |> sep
end
...
function fit!(svm::SVMClassifier, 
    ...
    for i in 1:max_iter
        ...
        begin
            ...
            svm.b -= lr*(grad[1].b)
            svm.ξ -= lr*(grad[1].ξ)
            svm.ξ = max.(svm.ξ, 0) ##if negative we project it back to 0!
        end        
        if i % 200 == 0
...
fit!(svm, X_train, y_train; max_iter = 10000, C = 100.0, lr = 1e-10)
